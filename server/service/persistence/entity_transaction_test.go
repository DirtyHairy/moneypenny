...
func assertTransactionCount(t *testing.T, p Provider, expected uint64) {
	...
	assert.NoError(t, err, "failed to query transaction count: %s", err)
...
func TestInsertAndRetrieveTransaction(t *testing.T) {
	...
	assert.NoError(t, p.AddTransaction(t0), "insert failed: %s", err)
	assertTransactionCount(t, p, 1)

	var t1 *model.Transaction
	t1, err = p.GetTransactionById(t0.Id)

	assert.NoError(t, err, "failed to retrieve transaction: %s", err)
...
func TestGetAllTransactions(t *testing.T) {
	...
	assert.NoError(t, p.AddTransaction(t0), "insert failed: %s", err)
	assert.NoError(t, p.AddTransaction(t1), "insert failed: %s", err)
	...
	assert.NoError(t, err, "failed to query all transactions: %s", err)
